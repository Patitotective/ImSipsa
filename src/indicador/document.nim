...
proc generateDocumentImpl(dateFormat, inputPath: string) =
  ...
  info &"Archivo del documento: {filename}"
  ...
  indiChannel.send finishDocMsg()

proc generateDocument*(dateFormat, inputPath: string) {.thread.} = 
  try:
